...
// Configuració de registres ADS1294
...
// --- Funcions d’escriptura/lectura de registres ADS1294 ---
...
float moving_average(float nova) {
  moving_avg_buffer[moving_avg_index] = nova;
  moving_avg_index = (moving_avg_index + 1) % MOVING_AVG_WINDOW;
  float suma = 0;
  for (int j = 0; j < MOVING_AVG_WINDOW; j++) suma += moving_avg_buffer[j];
  return suma / MOVING_AVG_WINDOW;
}


// --- Càlcul RMS amb finestra ---
#define RMS_WINDOW 30
float finestra_rms[RMS_WINDOW];
int idx_rms = 0;
bool finestra_plena = false;

float calcularRMS(float nova_mostra) {
  finestra_rms[idx_rms] = nova_mostra;
  idx_rms = (idx_rms + 1) % RMS_WINDOW;
  if (idx_rms == 0) finestra_plena = true;

  float suma = 0;
  int finestra_actual = finestra_plena ? RMS_WINDOW : idx_rms;

  for (int k = 0; k < finestra_actual; k++) {
    suma += finestra_rms[k] * finestra_rms[k];
  }

  return sqrt(suma / finestra_actual);
}
...
void loop() {
  if (novaMostra) {
    ...
    float mv_sense_offset = removeOffset(mv);
    float mv_suau = moving_average(mv_sense_offset);
    float rms = calcularRMS(mv_suau);
